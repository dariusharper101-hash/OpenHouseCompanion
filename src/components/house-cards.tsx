...
export function HouseCard({ home }: { home: Home }) {
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-white font-semibold leading-tight">{home.street || "Untitled home"}</h3>
...
/**
 * The client-facing presentation: homes grouped by location, each group sorted
 * by days on market. Pure component — safe to render from server or client.
 */
export default function HousePresentation({ homes }: { homes: Home[] }) {
      ...
      {groups.map((group) => (
        <section key={group.neighborhood}>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {group.homes.map((home, i) => (
              <HouseCard key={`${homeSlug(home)}-${i}`} home={home} />
            ))}
          </div>
        </section>
      ))}
...
/** Agent contact card shown at the bottom of the list and builder preview. */
